...
static void schedRx2 (ostime_t delay, osjobcb_t func) {
    ...
    print("y4 \r\n");
    os_setTimedCallback(&LMIC.osjob, LMIC.rxtime - RX_RAMPUP, func,"funcGen");
}
...
static void txDone (ostime_t delay, osjobcb_t func) {
    ...
    os_setTimedCallback(&LMIC.osjob, LMIC.rxtime - RX_RAMPUP, func,"runff");
}
    ...
    if( LMIC.dataLen == 0 ) {
      nojoinframe:
        ...
        os_setTimedCallback(&LMIC.osjob, os_getTime()+delay,
                            ...
                            : FUNC_ADDR(runEngineUpdate),"runEngineU1"); // next step to be delayed
        return 1;
    }
...
static void processRx2DnData (xref2osjob_t osjob) {
    if( LMIC.dataLen == 0 ) {
        ...
        os_setTimedCallback(&LMIC.osjob,
                            ...
                            processRx2DnDataDelay,"proRx2Dn");
        return;
    }
    processDnData();
}
...
static void onBcnRx (xref2osjob_t job) {
    ...
    if( decodeBeacon() <= 0 ) {
        ...
        os_radio(RADIO_RXON);
            print("y8 \r\n");        
        os_setTimedCallback(&LMIC.osjob, LMIC.bcninfo.txtime, FUNC_ADDR(onBcnRx),"onBcnRx");
        return;
    }
...
static void startScan (void) {
    ...
    print("y9 \r\n");
    os_setTimedCallback(&LMIC.osjob, LMIC.rxtime, FUNC_ADDR(onBcnRx),"onBcnRx1");
    os_radio(RADIO_RXON);
}
...
bit_t LMIC_startJoining (void) {
    if( LMIC.devaddr == 0 ) {
        ...
        os_setCallback(&LMIC.osjob, FUNC_ADDR(startJoining),"stJoin");
        return 1;
    }
...
static void engineUpdate (void) {
        ...
        txbeg = now;
            ...
            if( jacc ) {
                ...
                if( LMIC.seqnoDn >= 0xFFFFFF80 ) {
                  ...
                  reset:
                    os_setCallback(&LMIC.osjob, FUNC_ADDR(runReset),"runReset");
                    return;
                }
  ...
  checkrx:
    if( (LMIC.opmode & OP_PINGINI) != 0 ) {
        ...
        if( rxschedNext(&LMIC.ping, now+RX_RAMPUP) ) {
            ...
            print("y1 \r\n");
            os_setTimedCallback(&LMIC.osjob, LMIC.rxtime - RX_RAMPUP, FUNC_ADDR(startRxPing),"startRxPing");
            return;
        }
    ...
    print("y2 \r\n");
    os_setTimedCallback(&LMIC.osjob, rxtime, FUNC_ADDR(startRxBcn),"startRxBcn");
    return;
  ...
  txdelay:
    ...
    print("y3 \r\n");
    os_setTimedCallback(&LMIC.osjob, txbeg-TX_RAMPUP, FUNC_ADDR(runEngineUpdate),"rEngineUp");
    print("eu e \r\n");
}
