...
extern unsigned int dbg_radio_irq;
...
static void opmode (u1_t mode) {
    // print_s_decimal("--> opmode: ",mode);
    // writeReg(RegOpMode, (readReg(RegOpMode) & ~OPMODE_MASK) | mode);
    u1_t t = (readReg(RegOpMode) & ~OPMODE_MASK) | mode;
    writeReg(RegOpMode, t );
    // print_s_hex8("++> opmode: ",t);    
}
...
static void configChannel () {
    ...
    print_s_decimal("freq: ",LMIC.freq);
...
void radio_irq_handler (u1_t dio) {
    print_s_decimal("radio_irq_handler", dio );
    println("");
    ...
    os_setCallback(&LMIC.osjob, LMIC.osjob.func,"radioIRQ");
}

void os_radio (u1_t mode) {
    hal_disableIRQs();
    print_s_decimal("os_radio ",mode);
    switch (mode) {
      case RADIO_RST:
        // put radio to sleep
        opmode(OPMODE_SLEEP);
