...
void print_decimaln (uint32_t i);
void print_decimal (uint32_t i);
...
static void usart_setup(void)
{
  ...
  USART_CR3(USART2) = (1<<12);  // ovrdis needed for GPS communication 
...
static uint8_t data2 = 0xff;

static uint8_t char_[0];
static uint16_t strPtr = 0;
...
static uint32_t dbgCr1 = 0;
static uint32_t dbgCr3 = 0;
static uint32_t dbgISR = 0;
static uint32_t dbgICR = 0;
...
void usart2_isr(void)
{
  ...
  nvic_disable_irq(NVIC_USART2_IRQ);
    ...
    dbgCr1 = USART_CR1(USART2);
    dbgCr3 = USART_CR3(USART2);
    dbgISR = USART_ISR(USART2);
    dbgICR = USART_ICR(USART2);


  if( USART_ISR(USART2) & USART_SR_ORE )
      USART_ICR(USART2) &= 0x08;

  if( USART_ISR(USART2) & USART_SR_FE )
      USART_ICR(USART2) &= 0x02;

  /* Check if we were called because of RXNE. */
  if (((USART_CR1(USART2) & USART_CR1_RXNEIE) != 0) &&
      ((USART_ISR(USART2) & USART_SR_RXNE) != 0)) {


      isrC++;

    /* Retrieve the data from the peripheral. */
    data2 = usart_recv(USART2);

    if( data2 != 0x0 && data2 != 0xff ) {

      strt_[strPtr] = data2;
      strPtr = (strPtr < 500) ? strPtr + 1 : 0;
...
void smt32_gps(int argc, char **argv)
{
...
int endP = 2000;
    ...
    do {
       ...
       // if( ! strncmp( strt_+c,"$GNGGA",6)  )
          lastStartP = c;

       if( lastStartP != 2000 && strt_[c] == '*')      
          endP = c;
 
      // } while( c++ < 450 && lastStartP == 2000 );
...
tinysh_cmd_t cmd_gps = {0,"gps","gps data ","[...]",smt32_gps,0,0,0};
...
void print_s_decimal(char *s, uint32_t i) {
    print(s);
    print_decimal(i);
    print("\r\n");
