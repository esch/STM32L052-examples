...
char *strDR[3] = { "1MBPS", "2MBPS" , "250kBPS" };
char *strCRC[3] = { "RF24_CRC_DISABLED", "RF24_CRC_8" , "RF24_CRC_16" };


#ifdef LATER
#define PIN_SPI_MOSI               GPIO7      //
#define PORT_SPI_MOSI              GPIOA

#define PIN_SPI_MISO               GPIO6      
#define PORT_SPI_MISO              GPIOA

#define PIN_SPI_CLK               GPIO5      //SCK = PE12
#define PORT_SPI_CLK              GPIOA

#define PIN_SPI_CE                GPIO3     //
#define PORT_SPI_CE               GPIOA

#define PIN_SPI_CSN               GPIO4      //
#define PORT_SPI_CSN              GPIOA
#endif

#define PIN_SPI_MOSI               GPIO5      //
#define PORT_SPI_MOSI              GPIOB

#define PIN_SPI_MISO               GPIO4      
#define PORT_SPI_MISO              GPIOB

#define PIN_SPI_CLK               GPIO3      //SCK 
#define PORT_SPI_CLK              GPIOB

#define PIN_SPI_CE                GPIO6     //
#define PORT_SPI_CE               GPIOA

#define PIN_SPI_CSN               GPIO15      //
#define PORT_SPI_CSN              GPIOA
/****************************************************************************/
  ...
  pipe0_reading_address(0)
{
  ...
  //   spi.format(8,1);                                   // 8-bit, ClockPhase = 0, ClockPolarity = 0
}
...
void RF24::ce(int level)
{
  ...
  if(level)
   gpio_set(PORT_SPI_CE,PIN_SPI_CE);
  else 
   gpio_clear(PORT_SPI_CE,PIN_SPI_CE);
}
...
uint8_t RF24::read_register(uint8_t reg, uint8_t* buf, uint8_t len)
{
  ...
  // status = spi.write( R_REGISTER | ( REGISTER_MASK & reg ) );
   spi_send8_sw(SPI1, R_REGISTER | ( REGISTER_MASK & reg ) );
   spi_read8_sw(SPI1);

  while ( len-- ) {
    spi_send8_sw(SPI1,0xff);
    *buf++ = spi_read8_sw(SPI1);
  }
    // *buf++ = spi.write(0xff);
...
uint8_t RF24::read_register(uint8_t reg)
{
  ...
  spi_read8_sw(SPI1);
  spi_send8_sw(SPI1, 0xff);
  uint8_t result = spi_read8_sw(SPI1);
...
uint8_t RF24::read_register_x(uint8_t reg)
{
  csn(LOW);
  // spi.write( R_REGISTER | ( REGISTER_MASK & reg ) );
...
uint8_t RF24::write_register(uint8_t reg, const uint8_t* buf, uint8_t len)
{
  ...
  csn(LOW);
  // status = spi.write(  );
    spi_send8_sw(SPI1, W_REGISTER | ( REGISTER_MASK & reg ));
  while ( len-- )
      spi_send8_sw(SPI1, *buf++);
...
uint8_t RF24::write_payload(void* buf, uint8_t len)
{
  ...
  // print_s_decimal("write_payload blanks ",data_len);
  // print_s_decimal("write_payload blanks size",payload_size);
  //printf("[Writing %u bytes %u blanks]",data_len,blank_len);
  
  csn(LOW);
  // status = spi.write( W_TX_PAYLOAD );
   spi_send8_sw(SPI1, W_TX_PAYLOAD);
  while ( data_len-- )
     spi_send8_sw(SPI1,*reg++);

  while ( blank_len-- )
   spi_send8_sw(SPI1, 0x1);    
    // spi.write(0);
  csn(HIGH);
...
uint8_t RF24::read_payload(void* buf, uint8_t len)
{
  ...
  csn(LOW);
   spi_send8_sw(SPI1, R_RX_PAYLOAD);
   spi_read8_sw(SPI1);
  while ( data_len-- ) {
    spi_send8_sw(SPI1,0xff);
    *current++ = spi_read8_sw(SPI1);
    }
  while ( blank_len-- )
   spi_send8_sw(SPI1, 0xff);    
...
void RF24::print_status(uint8_t status)
{
  ...
  print("STATUS = "); print_hex8(status);
  print(" RX_DR = ");  print_hex8( (status & _BV(RX_DR))?1:0);
  print(" TX_DR = ");  print_hex8( (status & _BV(TX_DS))?1:0);
  print(" MAX_RT = "); print_hex8( (status & _BV(MAX_RT))?1:0);
  print(" RX_P_NO = "); print_hex8( ((status >> RX_P_NO) & 7)) ;
  print(" TX_FULL = "); print_hex8( (status & _BV(TX_FULL))?1:0 );
...
void RF24::setChannel(uint8_t channel)
{
  ...
  print_s_decimal("setChannel ",channel);
...
//  pinMode(csn_pin,OUTPUT);
  ...
  gpio_mode_setup(PORT_SPI_CSN, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, PIN_SPI_CSN);    // NSS
  gpio_set(PORT_SPI_CSN, PIN_SPI_CSN);
  gpio_clear(PORT_SPI_CSN, PIN_SPI_CSN);
  gpio_set(PORT_SPI_CSN, PIN_SPI_CSN);

  gpio_mode_setup(PORT_SPI_CE, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLDOWN, PIN_SPI_CE);    // NSS
  gpio_set(PORT_SPI_CE,PIN_SPI_CE);

// SPI
  gpio_mode_setup(PORT_SPI_MOSI, GPIO_MODE_AF, GPIO_PUPD_NONE,     PIN_SPI_MOSI );
  gpio_mode_setup(PORT_SPI_MISO, GPIO_MODE_AF, GPIO_PUPD_PULLUP,  PIN_SPI_MISO );
  gpio_mode_setup(PORT_SPI_CLK,  GPIO_MODE_AF, GPIO_PUPD_PULLUP,  PIN_SPI_CLK  );
  gpio_set_af(PORT_SPI_CLK, GPIO_AF0,    PIN_SPI_CLK | PIN_SPI_MISO | PIN_SPI_MOSI );
  ...
  // read_register_x(FEATURE);
  // write_register(FEATURE,0x4 );
  // read_register_x(FEATURE);

  // for(int x=0;x<8;x++)
  //   usart_send_blocking(USART1, sss[x]);
...
bool RF24::write( void* buf, uint8_t len )
{
  bool result = false;
...
//esch to be done  uint32_t sent_at = mainTimer.read_ms();

  // const uint32_t timeout = 500; //ms to wait for timeout
   uint32_t timeout = 10; //ms to wait for timeout
  do
  {
    status = read_register(OBSERVE_TX,&observe_tx,1);
   // IF_SERIAL_DEBUG(Serial.print(observe_tx,HEX));
// review !!! we still go into timeout !!!!
   // print_s_hex8("write : ",observe_tx);
  }
   while( ! ( status & ( _BV(TX_DS) | _BV(MAX_RT) ) ) &&  timeout--  );
...
uint8_t RF24::getDynamicPayloadSize(void)
{
  ...
  // result = spi.write(0xff);
     ...
     spi_send8_sw(SPI1, R_RX_PL_WID);
  // spi.write(value);
    result = spi_read8_sw(SPI1);
...
{
  csn(LOW);
  spi_send8_sw(SPI1, ACTIVATE);
  spi_send8_sw(SPI1, 0x73);
...
void RF24::enableDynamicPayloads(void)
{
  ...
  uint8_t rg = read_register_x(FEATURE);

  // print_s_decimal("enableDynamicPayloads: ",rg);
  write_register(FEATURE, rg | _BV(EN_DPL));

  // If it didn't work, the features are not enabled
  if ( ! read_register_x(FEATURE) )
  {
...
//  IF_SERIAL_DEBUG(printf("FEATURE=%i\r\n",read_register(FEATURE)));
  ...
  println("enableDynamicPayloads on !!!");
...
void RF24::writeAckPayload(uint8_t pipe, const void* buf, uint8_t len)
{
  ...
  csn(LOW);
  spi_send8_sw(SPI1,  W_ACK_PAYLOAD | ( pipe & 7 ) );
...
bool RF24::setDataRate(rf24_datarate_e speed)
{
  ...
  uint8_t setup = read_register_x(RF_SETUP) ;
  ...
  print_s_decimal("setDataRate ",speed);
  write_register(RF_SETUP,setup);
...
void RF24::setCRCLength(rf24_crclength_e length)
{
 ...
 print_s_hex8("setCRCLength ",length);
 ...
 // print_s_hex8("setCRCLength out ",config);
